...
class JmsSink {
    ...
    JmsSink(JMSContext context, JmsConnectorOutgoingConfiguration config, ObjectMapper jsonb, Executor executor) {
        String name = config.getDestination().orElseGet(config::getChannel);
        commit = config.getTransactionGroup().isPresent();
    ...
    private CompletionStage<Message<?>> send(Message<?> message) throws JMSException {
        ...
        if (payload instanceof String || payload.getClass().isPrimitive() || isPrimitiveBoxed(payload.getClass())) {
            ...
            try {
                outgoing = context.createTextMessage(json.writeValueAsString(payload));
            } catch (Exception ex) {
