...
import static org.lorislab.quarkus.reactive.jms.OutputJmsMessageMetadata.OutputJmsMessageMetadataBuilder.QUEUE;
import static org.lorislab.quarkus.reactive.jms.OutputJmsMessageMetadata.OutputJmsMessageMetadataBuilder.TOPIC;

public class InputJmsMessage<T> implements org.eclipse.microprofile.reactive.messaging.Message<T> {
    ...
    InputJmsMessage(JMSContext context, Message message, Executor executor, Jsonb json) {
    ...
    private Class<T> load(String cn) throws ClassNotFoundException {
        ...
        return (Class<T>) InputJmsMessage.class.getClassLoader().loadClass(cn);
    }
    ...
    private static void send(JMSContext context, Jsonb json, JMSProducer producer, org.eclipse.microprofile.reactive.messaging.Message<?> message) {
        OutputJmsMessageMetadata outgoingMetadata = message.getMetadata(OutputJmsMessageMetadata.class).orElse(null);
        ...
        try {
            ...
            throw new IllegalStateException("Error send the message!", ex);
        }
    }
    ...
    private static Message createJmsMessage(JMSContext context, Jsonb json, Object payload) throws JMSException {
